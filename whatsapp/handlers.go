...
func MessageFromOthersEventHandler(text string, v *events.Message, isEdited bool) {
			...
			if mentioned := contextInfo.GetMentionedJID(); v.Info.IsGroup && mentioned != nil {
				for _, jid := range mentioned {
					...
					if parsedJid.User == waClient.Store.ID.User {
						...
						threadId, err := utils.TgGetOrMakeThreadFromWa("mentions", "Mentions")
						if err != nil {
	...
	if !threadIdFound {
		var err error
		if v.Info.Chat.String() == "status@broadcast" {
			threadId, err = utils.TgGetOrMakeThreadFromWa("status@broadcast", "Status")
			if err != nil {
...
func PictureEventHandler(v *events.Picture) {
	...
	if cfg.WhatsApp.CreateThreadForInfoUpdates {
		...
		tgThreadId, err := utils.TgGetOrMakeThreadFromWa("coco-info-update@broadcast", "Info Updates")
		if err != nil {
			logger.Warn(
...
func GroupInfoEventHandler(v *events.GroupInfo) {
	...
	if !threadFound {
		...
		if cfg.WhatsApp.CreateThreadForInfoUpdates {
			_, err := utils.TgGetOrMakeThreadFromWa(v.JID.ToNonAD().String(), utils.WaGetGroupName(v.JID))
			if err != nil {
				logger.Warn(
